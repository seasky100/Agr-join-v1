...
var slipjs = {
	_refreshCommon: function(core_width, width) { // 作用：当尺寸改变时，需要重新取得相关的值
		var that = this;
		that.core_width = core_width || that.core[that.offset] - that.up_range;
		that.width      = width      || that.core.parentNode[that.offset];
	...
	_initCommon: function(core,param) { // 作用：初始化
		...
		that.core_width       = param.core_width || that.core[that.offset] - that.up_range;
		that.width            = param.width      || that.core.parentNode[that.offset];
		that._getCoreWidthSubtractShellWidth();
	...
	_getCoreWidthSubtractShellWidth: function() { // 作用：取得滑动对象和它父级元素的宽度或者高度的差
		var that = this;
		that.width_cut_coreWidth = that.width - that.core_width;
		that.coreWidth_cut_width = that.core_width - that.width;
	},
...
var SlipPage = _fun.clone(slipjs);
	...
	SlipPage.toPage = function(num, time) { // 可在外部调用的函数，指定轮换到第几张，只要传入：“轮换到第几张”和“时间”两个参数。
		this._posTime(-this.width * num, time || 0);
	};
	...
	SlipPage.refresh = function(core_width, width) { // 可在外部调用，作用：当尺寸改变时（如手机横竖屏时），需要重新取得相关的值。这时候就可以调用该函数
		this._refreshCommon(core_width, width);
	};
...
var SlipPx = _fun.clone(slipjs);
		...
		if (!this._movestart) {
			...
			if(duration < 300 && Math.abs(fast_dist_x) > 10) {
				if (that.xy > -that.up_range || that.xy < that.width_cut_coreWidth) {
					...
					var _momentum = that._momentum(fast_dist_x, duration, -that.xy - that.up_range, that.coreWidth_cut_width + (that.xy), that.width);
	...
	SlipPx._countBarSize = function() { // 作用：根据比例计算滚动条的高度
		this._bar_size = Math.round(Math.max(this.width * this.width / this.core_width, 5));
	};
	...
	SlipPx._countAboutBar = function() { // 作用：计算一个关于滚动条的的数值
		this._about_bar = ((this.width-4) - (this.width-4) * this.width / this.core_width)/this.width_cut_coreWidth;
	};
	SlipPx._countWidthCutBarSize = function() { // 作用：计算一个关于滚动条的的数值
		this._width_cut_barSize = (this.width-4) - this._bar_size;
	};
	SlipPx.refresh = function(core_width, width) {// 可在外部调用，作用：当尺寸改变时（如手机横竖屏时），需要重新取得相关的值。这时候就可以调用该函数
		var that = this;
		that._refreshCommon(core_width, width);
		if(!that.no_bar){
			if(that.coreWidth_cut_width <= 0) {
				that._bar_shell_opacity   = 0;
				that._showBar       = function(){};	
	...
	SlipPx._hideBar = function() {// 作用：隐藏滚动条
		...
		that._bar_shell.style.webkitTransitionDuration = that.duration+'ms';
	};
